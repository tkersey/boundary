...
theorem installHandler_unique (machine : State) (context : Context) (handler : HandlerId .source)
  ...
  have heap : Unique store := by
    ...
    exact allocate_nonregion_unique _ _ _ _ _ _ _ allocated rfl bounded
...
theorem enterRegion_unique (machine : State) (context : Context) (descriptor : RegionId .source)
    ...
    (bounded : IdentitySupport.Valid machine)
  ...
  have fresh : ∀ fresh ∈ some (⟨machine.heap.nextRegion⟩ : RegionInstanceId),
      ∀ node, atNode machine.heap node ≠ some fresh := by
    intro fresh member node collision
    cases member
    have earlier := identity_bounded bounded.heap collision
    exact Nat.lt_irrefl _ earlier
  have next := allocate_unique _ _ _ _ _ _ _ allocated formed fresh
