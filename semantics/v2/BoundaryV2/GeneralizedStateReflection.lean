...
theorem ExecutionStep.close_operand_result
    ...
    ∃ changed operands,
      OwnedOperandStep view.bindings (before.cells.reservations.withSupport retained).custody before.control.store view.operands.operands changed operands ∧
...
theorem ordinary_cancel_stateful_observed
    ...
    (expected : CallStep table before.control.configuration next)
    (neutral : before.control.configuration.needsOwnershipStep = false)
    (run : ExecutionSteps (retained := retained) table before count final)
    (head : HeadObservation final.control.configuration observation) :
    ∃ remaining, count = remaining + 1 ∧
      ExecutionSteps (retained := retained) table ⟨⟨before.control.store, next⟩, before.cells, before.liveRegions⟩ remaining final := by
  cases run with
...
/-- Local target facts used to invert compiled operand prefixes. The remaining
execution is a predicate so a control receiver may continue in its owning
driver. Neither field assumes a source evaluation or a translation theorem. -/
structure OperandTraceLaws (table : Definitions signature algebra program) (retained : List Reference)
    (run : State signature algebra program result → Nat → Prop) : Prop where
  ordinary : ∀ {before count next},
    CallStep table before.control.configuration next →
    before.control.configuration.needsOwnershipStep = false → run before count →
      ∃ remaining, count = remaining + 1 ∧
        run ⟨⟨before.control.store, next⟩, before.cells, before.liveRegions⟩ remaining
  close : ∀ {before count view}, before.control.configuration.closeView = some view → run before count →
    ∃ remaining after, count = remaining + 1 ∧ ExecutionStep table before after retained ∧ run after remaining

omit [DecidableEq (ControlShape signature)] [DecidableEq (TypeOf signature)] in
theorem HeadObservation.close_view_none
    (head : HeadObservation (configuration : Configuration signature algebra program result) observation) :
    configuration.closeView = none := by cases head <;> rfl

theorem core_operand_trace_laws (table : Definitions signature algebra program)
    (final : State signature algebra program result) (observation : Observation signature algebra program result) :
    OperandTraceLaws table retained (fun before count =>
      ExecutionSteps table before count final retained ∧ HeadObservation final.control.configuration observation) := by
  constructor
  · intro before count next expected neutral observed
    obtain ⟨remaining, counted, tail⟩ := ordinary_cancel_stateful_observed expected neutral observed.1 observed.2
    exact ⟨remaining, counted, tail, observed.2⟩
  · intro before count view viewed observed
    obtain ⟨run, head⟩ := observed
    cases run with
    | refl => rw [head.close_view_none] at viewed; cases viewed
    | cons step tail => exact ⟨_, _, rfl, step, tail, head⟩
...
theorem ExecutionSteps.returned_done
    ...
    (run : ExecutionSteps (retained := retained) table ⟨⟨store, .returned value .done⟩, storage, regions⟩ count final) :
...
theorem ExecutionSteps.failed_done
    ...
    (run : ExecutionSteps (retained := retained) table (⟨⟨store, .failed fault .done⟩, storage, regions⟩ : State signature algebra program result) count final) :
...
variable {signature : Signature} {algebra : LeafAlgebra signature.Data}
  {program : List (BodyType signature.Data signature.Effect)} {retained : List Reference} {SourceFuture TargetFuture : Type}
...
theorem closure_state_step_reflected
    ...
    (selection : Selection context capturedTypes)
    (body : Source.Computation signature algebra program (parameters ++ capturedTypes) answer)
    {sourceStore : UseScope.ControlStore SourceFuture}
    ...
    ∃ sourceValue sourceAfter changed,
      Source.ClosureEvaluation bindings (storage.reservations.withSupport retained).custody (.lambda (use := use) selection body)
  ...
  obtain ⟨sourceValue, sourceAfter, evaluated, matched, valuesAt⟩ :=
    closure_operand_reflected futureRelation bindings selection body
      (storage.reservations.withSupport retained).custody stores next values owned
...
theorem selection_cancel_operand_trace
    ...
    (laws : Target.OperandTraceLaws table retained trace)
    (run : trace
      ⟨⟨store, .code (selection captures next) bindings values outside⟩, storage, regions⟩ count)
    :
    ∃ remaining, remaining ≤ count ∧
      trace
        ⟨⟨store, .code next bindings ((bindings.select captures).pushReverse values) outside⟩, storage, regions⟩ remaining := by
  ...
  | cons reference rest induction =>
    ...
    obtain ⟨remaining, decreased, tail⟩ := laws.ordinary first rfl run
...
theorem lambda_operand_trace_reflected
    (table : Source.Definitions signature algebra program)
    (futureRelation : SourceFuture → Sigma (Target.ControlPayload signature algebra program) → Prop)
    (bindings : Source.RuntimeEnvironment signature algebra program context)
    ...
    (run : trace
      ⟨⟨targetStore, .code (expression (.lambda captures body) next) (environment bindings) values outside⟩, cells storage, regions⟩ count)
    :
    ∃ sourceValue sourceAfter changed remaining,
      Source.ExpressionEvaluation bindings (storage.reservations.withSupport retained).custody sourceStore (.lambda (use := use) captures body) (.ok sourceValue) sourceAfter ∧
      UseScope.ControlStore.Related futureRelation sourceAfter changed ∧ remaining < count ∧
      trace
        ⟨⟨changed, .code next (environment bindings) (.cons (value sourceValue) values) outside⟩, cells storage, regions⟩ remaining := by
  obtain ⟨remaining, bounded, tail⟩ := selection_cancel_operand_trace captures (environment bindings)
    (.close (computation body) next) values targetStore (cells storage) regions outside (definitions table) laws run
  ...
  rw [selected] at tail
  obtain ⟨rest, after, counted, step, following⟩ := laws.close
    (Target.close_configuration_view (computation body) (environment (bindings.select captures)) next values (environment bindings) outside) tail
  obtain ⟨sourceValue, sourceAfter, changed, evaluated, matched, configuration⟩ :=
    closure_state_step_reflected table futureRelation bindings captures body stores storage regions next values outside step
  rw [configuration] at following
  exact ⟨sourceValue, sourceAfter, changed, rest, .closure evaluated, matched, by omega, following⟩

private theorem operand_trace_reflection_bounded
    (table : Source.Definitions signature algebra program)
    (futureRelation : SourceFuture → Sigma (Target.ControlPayload signature algebra program) → Prop)
    (bindings : Source.RuntimeEnvironment signature algebra program context)
    (storage : Cells signature algebra (Source.Computation signature algebra program))
    (regions : List (Id .region))
    {result : TypeOf signature} {trace : Target.State signature algebra program result → Nat → Prop}
    (laws : Target.OperandTraceLaws (definitions table) retained trace) (bound : Nat) :
    (∀ {type} (source : Source.Expression signature algebra program context type), sizeOf source < bound →
      ...
      UseScope.ControlStore.Related futureRelation sourceStore targetStore →
      ∀ {stack input} (next : Target.Code signature algebra program context (type :: stack) input)
      ...
      {count},
      trace
        ⟨⟨targetStore, .code (expression source next) (environment bindings) values outside⟩, cells storage, regions⟩ count →
      ∃ outcome sourceAfter changed remaining,
        Source.ExpressionEvaluation bindings (storage.reservations.withSupport retained).custody sourceStore source outcome sourceAfter ∧
        UseScope.ControlStore.Related futureRelation sourceAfter changed ∧ remaining < count ∧
        trace
          ⟨⟨changed, Target.expressionOutcome next (environment bindings) values outside (outcome.map value)⟩, cells storage, regions⟩ remaining) ∧
    (∀ {types} (source : Source.Arguments signature algebra program context types), sizeOf source < bound →
      ∀ {sourceStore : UseScope.ControlStore SourceFuture} {targetStore : Target.ControlHeap signature algebra program},
      UseScope.ControlStore.Related futureRelation sourceStore targetStore →
      ∀ {stack input} (next : Target.Code signature algebra program context (types.reverse ++ stack) input)
      (values : Target.RuntimeEnvironment signature algebra program stack) (outside : Target.Stack signature algebra program input result)
      {count},
      trace
        ⟨⟨targetStore, .code (arguments source next) (environment bindings) values outside⟩, cells storage, regions⟩ count →
      ∃ outcome sourceAfter changed remaining,
        Source.ArgumentsEvaluation bindings (storage.reservations.withSupport retained).custody sourceStore source outcome sourceAfter ∧
        UseScope.ControlStore.Related futureRelation sourceAfter changed ∧ remaining ≤ count ∧
        (∀ fault, outcome = .error fault → remaining < count) ∧
        trace
          ⟨⟨changed, Target.argumentsOutcome next (environment bindings) values outside (outcome.map environment)⟩, cells storage, regions⟩ remaining) := by
  cases bound with
  | zero => constructor <;> intro type source sized <;> omega
  | succ bound =>
    obtain ⟨expressions, argumentLists⟩ := operand_trace_reflection_bounded table futureRelation bindings storage regions laws bound
    constructor
    · intro type source sized sourceStore targetStore stores stack input next values outside count run
      cases source with
      | datum datum =>
        obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .push) rfl run
        exact ⟨.ok (.datum datum), sourceStore, targetStore, remaining, .datum, stores, by omega, tail⟩
      | reference reference =>
        obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .load) rfl run
      ...
      | lambda captures body =>
        obtain ⟨sourceValue, sourceAfter, changed, remaining, evaluated, matched, smaller, tail⟩ :=
          lambda_operand_trace_reflected table futureRelation bindings captures body stores storage regions next values outside laws run
      ...
      | pair first second =>
        obtain ⟨firstOutcome, middleSource, middleTarget, middleCount, firstEval, middleRelated, firstLess, middleRun⟩ :=
          expressions first (by simp_all; omega) stores (expression second (.pair next)) values outside run
        ...
        | ok firstValue =>
          obtain ⟨secondOutcome, afterSource, afterTarget, afterCount, secondEval, afterRelated, secondLess, afterRun⟩ :=
            expressions second (by simp_all; omega) middleRelated (.pair next) (.cons (value firstValue) values) outside middleRun
          ...
          | ok secondValue =>
            obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .pair) rfl afterRun
      ...
      | first operand =>
        obtain ⟨outcome, sourceAfter, changed, rest, evaluated, matched, smaller, following⟩ :=
          expressions operand (by simp_all; omega) stores (.first next) values outside run
        cases outcome with
        | error fault => exact ⟨.error fault, sourceAfter, changed, rest, .firstFault evaluated, matched, smaller, following⟩
        | ok pair =>
          obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .first) rfl following
      ...
      | second operand =>
        obtain ⟨outcome, sourceAfter, changed, rest, evaluated, matched, smaller, following⟩ :=
          expressions operand (by simp_all; omega) stores (.second next) values outside run
        cases outcome with
        | error fault => exact ⟨.error fault, sourceAfter, changed, rest, .secondFault evaluated, matched, smaller, following⟩
        | ok pair =>
          obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .second) rfl following
      ...
      | left operand =>
        obtain ⟨outcome, sourceAfter, changed, rest, evaluated, matched, smaller, following⟩ :=
          expressions operand (by simp_all; omega) stores (.left next) values outside run
        cases outcome with
        | error fault => exact ⟨.error fault, sourceAfter, changed, rest, .leftFault evaluated, matched, smaller, following⟩
        | ok inner =>
          obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .left) rfl following
          exact ⟨.ok (.left inner), sourceAfter, changed, remaining, .left evaluated, matched, by omega, tail⟩
      | right operand =>
        obtain ⟨outcome, sourceAfter, changed, rest, evaluated, matched, smaller, following⟩ :=
          expressions operand (by simp_all; omega) stores (.right next) values outside run
        cases outcome with
        | error fault => exact ⟨.error fault, sourceAfter, changed, rest, .rightFault evaluated, matched, smaller, following⟩
        | ok inner =>
          obtain ⟨remaining, counted, tail⟩ := laws.ordinary (Target.CallStep.operand .right) rfl following
      ...
      | primitive operation inputs =>
        obtain ⟨outcome, sourceAfter, changed, rest, evaluated, matched, smaller, faultSmaller, following⟩ :=
          argumentLists inputs (by simp_all; omega) stores (.primitive operation next) values outside run
        cases outcome with
        | error fault =>
        ...
        | ok arguments =>
          ...
          | ok resultValue =>
            ...
            obtain ⟨remaining, counted, tail⟩ := laws.ordinary
              (Target.CallStep.operand (.primitive targetComputed)) rfl following
          ...
          | error fault =>
            have targetComputed : algebra.evaluate operation (environment arguments).leaves = .error fault := by
              simpa only [environment, Environment.leaves_map] using computed
            obtain ⟨firstRest, firstCount, faultRun⟩ := laws.ordinary
              (Target.CallStep.operand (.primitiveFault targetComputed)) rfl following
            obtain ⟨remaining, counted, tail⟩ := laws.ordinary Target.CallStep.fault rfl faultRun
            exact ⟨.error fault, sourceAfter, changed, remaining, .primitiveFault evaluated computed, matched, by omega, tail⟩
    · intro types source sized sourceStore targetStore stores stack input next values outside count run
      cases source with
      ...
      | cons first rest =>
        let nextRest : Target.Code signature algebra program context (_ ++ _ :: stack) input := by
          simpa only [List.reverse_cons, List.append_assoc, List.singleton_append] using next
        obtain ⟨firstOutcome, middleSource, middleTarget, middleCount, firstEval, middleRelated, firstLess, middleRun⟩ :=
          expressions first (by simp_all; omega) stores (arguments rest nextRest) values outside run
        cases firstOutcome with
        ...
        | ok firstValue =>
          obtain ⟨restOutcome, afterSource, afterTarget, afterCount, restEval, afterRelated, restLess, restFaultLess, afterRun⟩ :=
            argumentLists rest (by simp_all; omega) middleRelated nextRest (.cons (value firstValue) values) outside middleRun
...
theorem expression_trace_reflected
    (table : Source.Definitions signature algebra program)
    (futureRelation : SourceFuture → Sigma (Target.ControlPayload signature algebra program) → Prop)
    (bindings : Source.RuntimeEnvironment signature algebra program context)
    (storage : Cells signature algebra (Source.Computation signature algebra program)) (regions : List (Id .region))
    ...
    ∃ outcome sourceAfter changed remaining,
      ...
      trace
        ⟨⟨changed, Target.expressionOutcome next (environment bindings) values outside (outcome.map value)⟩,
          cells storage, regions⟩ remaining :=
  (operand_trace_reflection_bounded table futureRelation bindings storage regions laws (sizeOf source + 1)).1
    source (Nat.lt_succ_self _) stores next values outside run

theorem arguments_trace_reflected
    (table : Source.Definitions signature algebra program)
    (futureRelation : SourceFuture → Sigma (Target.ControlPayload signature algebra program) → Prop)
    (bindings : Source.RuntimeEnvironment signature algebra program context)
    (storage : Cells signature algebra (Source.Computation signature algebra program)) (regions : List (Id .region))
    ...
    ∃ outcome sourceAfter changed remaining,
      ...
      trace
        ⟨⟨changed, Target.argumentsOutcome next (environment bindings) values outside (outcome.map environment)⟩,
          cells storage, regions⟩ remaining :=
  (operand_trace_reflection_bounded table futureRelation bindings storage regions laws (sizeOf source + 1)).2
    source (Nat.lt_succ_self _) stores next values outside run
...
theorem expression_observing_run_reflected
  ...
  obtain ⟨outcome, sourceAfter, changed, remaining, evaluated, matched, smaller, following⟩ :=
    expression_trace_reflected table (UseScope.PackedControlRelated controlPayloadRelated) bindings storage regions source stores
      next values outside (Target.core_operand_trace_laws (definitions table) final observation) ⟨run, head⟩
  exact ⟨outcome, sourceAfter, changed, remaining, evaluated, matched, smaller, following.1⟩
...
theorem arguments_observing_run_reflected
    ...
    ∃ outcome sourceAfter changed remaining,
      ...
      Target.ExecutionSteps (retained := retained) (definitions table)
        ⟨⟨changed, Target.argumentsOutcome next (environment bindings) values outside (outcome.map environment)⟩,
          cells storage, regions⟩ remaining final := by
  obtain ⟨outcome, sourceAfter, changed, remaining, evaluated, matched, smaller, faultSmaller, following⟩ :=
    arguments_trace_reflected table (UseScope.PackedControlRelated controlPayloadRelated) bindings storage regions source stores
      next values outside (Target.core_operand_trace_laws (definitions table) final observation) ⟨run, head⟩
  exact ⟨outcome, sourceAfter, changed, remaining, evaluated, matched, smaller, faultSmaller, following.1⟩
...
theorem computation_operands_observing_run_reflected
    ...
    (head : Target.HeadObservation final.control.configuration observation) :
    ∃ outcome sourceAfter changed remaining,
      Source.ArgumentsEvaluation bindings (storage.reservations.withSupport retained).custody sourceStore body.operandPrefix.arguments outcome sourceAfter ∧
      ControlHeapRelated sourceAfter changed ∧ remaining ≤ count ∧
      (∀ fault, outcome = .error fault → remaining < count) ∧
      Target.ExecutionSteps (retained := retained) (definitions table)
...
theorem returned_expression_observation_reflected
    ...
    (observed : Target.StateObserves (retained := retained) (definitions table)
    ...
    ∃ sourceFinal sourceObservation,
      Source.StateObserves (retained := retained) table ⟨⟨sourceStore, .evaluate (.returnValue expression) bindings⟩, storage, regions⟩ sourceFinal sourceObservation ∧
