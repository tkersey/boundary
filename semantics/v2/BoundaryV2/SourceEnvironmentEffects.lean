...
theorem heapPrimitive_preserves_types (machine : State) (context : Context)
  ...
  case cellNew =>
    ...
    have allocatedTyped := allocateObject_preserves_types _
      ...
      _ _ _ _ _ _ allocated movedTyped (by simp [object, Environment.Types])
  ...
  case cellSet =>
    ...
    have storeTyped := replaceObject_preserves_types _ _ _ _ _ replaced typed (by simp [object, Environment.Types])
  ...
  case package =>
    ...
    have middleTyped := temporary_preserves_types _ _ _ _ temporaryOk typed
    have movedTyped := moveValues_preserves_types _ _ _ _ _ moveOk middleTyped
    have allocatedTyped := allocateObject_preserves_types _ _ _ _ _ _ _ _ allocated movedTyped (by simp [object, Environment.Types])
    exact finishTemporary_preserves_types _ _ _ _ finished allocatedTyped
  case unpack =>
    split at accepted <;> try contradiction
    simp only [except_bind_ok] at accepted
  ...
  case resourcePack =>
    ...
    have middleTyped := temporary_preserves_types _ _ _ _ temporaryOk typed
    have allocatedTyped := allocateObject_preserves_types _ _ _ _ _ _ _ _ allocated middleTyped (by simp [object, Environment.Types])
    exact finishTemporary_preserves_types _ _ _ _ finished allocatedTyped
  case resourceUnpack =>
    split at accepted <;> try contradiction
    simp only [except_bind_ok, fromOption_ok] at accepted
...
theorem completeHandler_preserves_types (machine : State) (context : Context) (after : Transition)
  ...
  simp only [All, Environment.Types, state, stacked, control, frame, List.flatMap_cons,
...
theorem enterRegion_preserves_types (machine : State) (context : Context) (descriptor : RegionId .source)
  ...
  have allocatedTyped := allocateObject_preserves_types _
    ...
    _ _ _ _ _ _ allocated typed (by simp [object, Environment.Types])
...
theorem handler_environment_types (context : Context) (handler : Handler .source) (bindings : Environment)
    (typed : Environment.Types context.source bindings) :
    Environment.Types context.source (handlerEnvironment context handler bindings) := by
  intro binding member
  exact typed binding (List.mem_filter.mp member).1

theorem trim_frame_types (context : Context) (saved : Frame)
    (typed : Environment.Types context.source (frame saved)) :
    Environment.Types context.source (frame (trimFrame context saved)) := by
  cases saved <;> simp only [trimFrame, frame] at typed ⊢
  all_goals first
    | exact typed
    | (intro binding member; exact typed binding (List.mem_filter.mp member).1)
...
theorem installHandler_preserves_types (machine : State) (context : Context) (handler : HandlerId .source)
  ...
  have allTyped := foldlM_preserves _ _ property (by
    ...
    exact allocateObject_preserves_types _ { machine with heap := heap }
      _ _ _ _ _ _ allocated beforeTyped (by simp [object, Environment.Types])) _ _ allocated seedTyped
  have selectedTyped := handler_environment_types context definition bindings outer
  have framedTyped : All context.source
    ...
    simp only [property, All, Environment.Types, state, List.flatMap_cons, frame, List.mem_append] at allTyped ⊢
    simp only [Environment.Types] at selectedTyped
    grind only []
...
theorem activateCapture_preserves_types (machine : State) (context : Context) (saved : Capture)
    ...
    (captureTyped : Environment.Types context.source (capture saved))
    (successorTyped : match successor with | none => True | some (_, _, bindings) => Environment.Types context.source bindings) :
  ...
  | none =>
    ...
    cases accepted
    split
    all_goals simp only [All, Environment.Types, state, List.flatMap_append, List.flatMap_cons,
  ...
  | some successor =>
    ...
    have selectedTyped := handler_environment_types context definition bindings successorTyped
    simp only [All, Environment.Types, state, Option.isSome_some, Bool.true_or, if_true,
...
theorem resumeValue_preserves_types (machine : State) (context : Context) (token argument : Located)
  ...
  have activeTyped := activateCapture_preserves_types _ _ _ _ _ activated capturedTyped.1 capturedTyped.2 (by cases successor <;> exact successorTyped)
...
theorem resumeComputation_preserves_types (machine : State) (context : Context)
  ...
  have capturedTyped := takeCapture_preserves_types _ _ _ _ captured typed
  have activeTyped := activateCapture_preserves_types _ _ _ _ _ activated capturedTyped.1 capturedTyped.2 trivial
...
theorem openRequest_preserves_types (machine : State) (context : Context)
  ...
  · simp only [except_bind_ok] at accepted
    ...
    have activationTyped : Environment.Types context.source selected.activation.environment := by
      intro binding member
      apply typed
      simp [state, reconstructed, frame, member]
    have outsideTyped : Environment.Types context.source (selected.outside.flatMap frame) := by
      intro binding member
      apply typed
    ...
    · simp only [except_bind_ok, fromOption_ok] at accepted
      ...
      have framesTyped : Environment.Types context.source ((selected.inside.map (trimFrame context)).flatMap frame) := by
        intro binding member
...
theorem executeEffectTerm_preserves_types (machine : State) (context : Context) (after : Transition)
  ...
  case handle =>
    split at accepted <;> try contradiction
    exact installHandler_preserves_types _ _ _ _ _ _ _ _ accepted typed outer
  case resumeValue =>
    split at accepted <;> try contradiction
    exact resumeValue_preserves_types _ _ _ _ _ _ accepted typed trivial
  case resumeWith =>
    split at accepted <;> try contradiction
    exact resumeValue_preserves_types _ _ _ _ _ _ accepted typed outer
