import BoundaryV2.SourceLinearityStorage
...
theorem enterRegion_preserves_linearity (machine : State) (context : Context) (descriptor : RegionId .source)
    ...
    (inputs : ∀ value ∈ arguments, Linear value.value) :
  ...
  have frameTyped : ValueInventory.All (Linear)
...
theorem installHandler_preserves_linearity (machine : State) (context : Context) (handler : HandlerId .source)
    ...
    (typed : ValueInventory.All (Linear) machine)
    (inputs : ∀ value ∈ arguments, Linear value.value)
    (storedTyped : ∀ value ∈ stored, Linear value.value)
    (outer : ∀ binding ∈ bindings, Linear binding.located.value) :
    ValueInventory.All (Linear) after.state := by
  ...
  let property (pair : Heap × List Located) : Prop :=
    ValueInventory.All (Linear) { machine with heap := pair.1 } ∧
    ∀ value ∈ pair.2, Linear value.value
  ...
  have framedTyped : ValueInventory.All (Linear)
...
theorem takeCapture_preserves_linearity (machine : State) (context : Context) (token : Located)
    (after : State × Capture) (accepted : takeCapture machine context token = .ok after)
    (typed : ValueInventory.All (Linear) machine) :
    ValueInventory.All (Linear) after.1 ∧
    (∀ value ∈ ValueInventory.capture after.2, Linear value) := by
...
theorem resumeValue_preserves_linearity (machine : State) (context : Context) (token argument : Located)
    ...
    (typed : ValueInventory.All (Linear) machine)
    (argumentTyped : Linear argument.value)
    (successorTyped : match successor with
      | none => True
      | some (_, stored, bindings) => (∀ value ∈ stored, Linear value.value) ∧
          (∀ binding ∈ bindings, Linear binding.located.value)) :
    ValueInventory.All (Linear) after.state := by
  ...
  have capturedTyped := takeCapture_preserves_linearity _ _ _ _ captured typed
...
theorem resumeComputation_preserves_linearity (machine : State) (context : Context)
...
theorem openRequest_preserves_linearity (machine : State) (context : Context)
    ...
    (typed : ValueInventory.All (Linear) machine)
    (inputs : ∀ value ∈ operands, Linear value.value) :
    ValueInventory.All (Linear) after.state := by
  ...
  · simp only [except_bind_ok] at accepted
    ...
    have activationTyped : ∀ value ∈ ValueInventory.activation selected.activation, Linear value := by
      intro value member
      apply typed
      simp [ValueInventory.state, reconstructed, ValueInventory.frame, member]
    have outer : ∀ binding ∈ selected.activation.environment, Linear binding.located.value := by
    ...
    have storedTyped : ∀ value ∈ selected.activation.state, Linear value.value := by
      intro value member
      apply activationTyped
      simp only [ValueInventory.activation, List.mem_append, List.mem_map]
      exact Or.inr ⟨value, member, rfl⟩
    have outsideTyped : ∀ value ∈ selected.outside.flatMap ValueInventory.frame, Linear value := by
      intro value member
      apply typed
    ...
    · simp only [except_bind_ok, fromOption_ok] at accepted
      ...
      all_goals
        ...
        have captureTyped : ∀ value ∈ ValueInventory.capture
          ...
          simp only [ValueInventory.capture, List.mem_append, List.mem_map]
          grind only []
        have startingTyped : ValueInventory.All (Linear)
...
/-- Every successful effect-term transition preserves per-value uniqueness,
including capture, cloning, and handler replacement. -/
theorem executeEffectTerm_preserves_linearity (machine : State) (context : Context) (after : Transition)
  ...
  case perform operation => exact openRequest_preserves_linearity _ _ _ _ _ accepted typed inputs
  case handle handler body arguments stored =>
    split at accepted <;> try contradiction
    rename_i closure rest operandsEqual
    apply installHandler_preserves_linearity _ _ _ _ _ _ _ _ accepted typed
    · intro value member
  ...
  case resumeValue token argument =>
    ...
    exact resumeValue_preserves_linearity _ _ _ _ _ _ accepted typed
  ...
  case resumeWith token argument handler stored =>
    ...
    apply resumeValue_preserves_linearity _ _ _ _ _ _ accepted typed (inputs argumentValue (by simp))
    exact ⟨fun value member => inputs value (by simp [member]), outer⟩
  case resumeComputation token computation =>
    split at accepted <;> try contradiction
    exact resumeComputation_preserves_linearity _ _ _ _ _ accepted typed
  case withRegion descriptor body arguments =>
    split at accepted <;> try contradiction
    rename_i closure rest operandsEqual
    exact enterRegion_preserves_linearity _ _ _ _ _ _ accepted typed
