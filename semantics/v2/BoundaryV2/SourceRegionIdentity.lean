...
def identity : Object → Option RegionInstanceId
  | .region region _ _ _ => some region
  | _ => none

def atNode (heap : Heap) (node : NodeId) : Option RegionInstanceId := (heap.lookup node).bind identity

/-- A logical region identity names one physical source region object. -/
def Unique (heap : Heap) : Prop := ∀ left right regionIdentity,
  atNode heap left = some regionIdentity → atNode heap right = some regionIdentity → left = right
...
theorem unique_of_restriction (formed : Unique before)
    (retained : ∀ node regionIdentity, atNode after node = some regionIdentity → atNode before node = some regionIdentity) : Unique after :=
  fun left right regionIdentity first second => formed left right regionIdentity (retained _ _ first) (retained _ _ second)
...
theorem initial_unique (context : Context) (arguments : List SemanticValue) (machine : State)
  ...
  intro left right regionIdentity found
...
theorem append_unique (formed : Unique heap)
    (fresh : ∀ regionIdentity ∈ identity stored, ∀ node, atNode heap node ≠ some regionIdentity) :
    Unique {heap with objects := heap.objects ++ [some stored]} := by
  intro left right regionIdentity first second
  obtain ⟨firstObject, firstAt, firstRegionIdentity⟩ := Option.bind_eq_some_iff.mp first
  obtain ⟨secondObject, secondAt, secondRegionIdentity⟩ := Option.bind_eq_some_iff.mp second
  ...
  · rcases append_lookup _ _ _ _ secondAt with secondOld | ⟨secondIndex, rfl⟩
    · exact formed _ _ _ (Option.bind_eq_some_iff.mpr ⟨_, firstOld, firstRegionIdentity⟩)
        (Option.bind_eq_some_iff.mpr ⟨_, secondOld, secondRegionIdentity⟩)
    · exact False.elim (fresh regionIdentity secondRegionIdentity left (Option.bind_eq_some_iff.mpr ⟨_, firstOld, firstRegionIdentity⟩))
  · rcases append_lookup _ _ _ _ secondAt with secondOld | ⟨secondIndex, rfl⟩
    · exact False.elim (fresh regionIdentity firstRegionIdentity right (Option.bind_eq_some_iff.mpr ⟨_, secondOld, secondRegionIdentity⟩))
...
theorem identity_bounded (bounded : IdentitySupport.HeapValid (IdentitySupport.limits heap) heap)
    (found : atNode heap node = some regionIdentity) : regionIdentity.value < heap.nextRegion := by
  obtain ⟨stored, storedAt, regionAt⟩ := Option.bind_eq_some_iff.mp found
...
theorem allocate_unique (heap after : Heap) (schema : SchemaId .source) (stored : Object)
    ...
    (formed : Unique heap) (fresh : ∀ regionIdentity ∈ identity stored, ∀ node, atNode heap node ≠ some regionIdentity) : Unique after := by
...
theorem replace_unique (heap after : Heap) (node : NodeId) (old stored : Object)
  ...
  intro selected regionIdentity found
