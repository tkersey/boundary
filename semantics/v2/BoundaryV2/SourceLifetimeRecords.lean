...
/-- These reusable records keep their exact physical identity and original
lifetime metadata. Their continued presence does not authorize use outside scope. -/
def signature : Object → Option Object
  | stored@(.capability ..) | stored@(.region ..) | stored@(.borrow ..) => some stored
  | _ => none

def view (objects : List (Option Object)) (node : NodeId) : Option Object :=
...
theorem retire_mutable_preserves (machine : State) (value : Located) (node : NodeId) (stored : Object) (after : Heap)
    (looked : lookupObject machine value = .ok (node, stored)) (untracked : signature stored = none)
  ...
  · have nodes : other = node := by cases other; cases node; simp_all
    ...
    simp only [atNode, Option.bind_some, untracked] at found
    cases found
...
theorem heapPrimitive_preserves (machine : State) (context : Context) (operation : Primitives.GraphOperation)
  ...
  case cellSet =>
    ...
    have first := replace_preserves _ _ _ _ _ atNode (by simp [signature]) replaced
  ...
  case cloneResumption =>
    ...
    have first := retire_mutable_preserves _ _ _ _ _ looked rfl retireOk
