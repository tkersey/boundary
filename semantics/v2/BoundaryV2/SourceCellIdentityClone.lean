...
private theorem rename_identity (mapping : Renaming) (object : Object) :
    identity (renameObject mapping object) = (identity object).map (renamed mapping.cells) := by
  cases object <;> rfl
...
theorem instantiateCapture_unique (machine : State) (context : Context) (saved : Capture) (after : State × Capture)
  ...
  have origin (node : NodeId) (cell : CellId) (found : atNode resultHeap node = some cell) :
      atNode machine.heap node = some cell ∨
      ∃ index original oldCell, copyNodes[index]? = some original ∧
        atNode machine.heap original = some oldCell ∧ oldCell ∈ cells ∧
        cell = renamed mapping.cells oldCell ∧ node.value = machine.heap.objects.length + index := by
    ...
    · have high := Nat.le_of_not_gt old
      obtain ⟨stored, found, cellAt⟩ := Option.bind_eq_some_iff.mp found
      ...
      | some storedEntry =>
        ...
        cases equal
        change identity (renameObject mapping (frozenObject saved dormant original originalObject)) = some cell at cellAt
        rw [rename_identity, frozen_identity] at cellAt
        cases originalObject <;> try contradiction
        rename_i oldCell schema region content
        simp only [identity, Option.map_some, Option.some.injEq] at cellAt
        refine Or.inr ⟨node.value - machine.heap.objects.length, original, oldCell, atInput, ?_, ?_, cellAt.symm, by omega⟩
        · simp [atNode, originalAt, identity]
        · exact List.mem_filterMap.mpr ⟨original, List.mem_of_getElem? atInput, by simp [originalAt]⟩
  intro left right cell first second
  rcases origin left cell first with firstOld | ⟨firstIndex, firstOriginal, firstCell, firstAt, firstCellAt, firstInside, firstRenamed, firstIndexEq⟩
  · rcases origin right cell second with secondOld | ⟨secondIndex, secondOriginal, secondCell, secondAt, secondCellAt, secondInside, secondRenamed, secondIndexEq⟩
    · exact formed _ _ _ firstOld secondOld
    · have before := identity_bounded bounded.heap firstOld
      have fresh := (source_activation_interval .runtime .cell machine.heap.nextCell cells secondCell secondInside).1
      change machine.heap.nextCell ≤ (renamed mapping.cells secondCell).value at fresh
      rw [← secondRenamed] at fresh
      omega
  · rcases origin right cell second with secondOld | ⟨secondIndex, secondOriginal, secondCell, secondAt, secondCellAt, secondInside, secondRenamed, secondIndexEq⟩
    · have before := identity_bounded bounded.heap secondOld
      have fresh := (source_activation_interval .runtime .cell machine.heap.nextCell cells firstCell firstInside).1
      change machine.heap.nextCell ≤ (renamed mapping.cells firstCell).value at fresh
      rw [← firstRenamed] at fresh
      omega
    · have sameCell := source_activation_injective .runtime .cell machine.heap.nextCell cells
        firstCell secondCell firstInside secondInside (firstRenamed.symm.trans secondRenamed)
      have sameNode := formed firstOriginal secondOriginal firstCell firstCellAt (sameCell ▸ secondCellAt)
...
theorem allocate_noncell_unique (heap after : Heap) (schema : SchemaId .source) (stored : Object)
    ...
    (noncell : identity stored = none) (formed : Unique heap) : Unique after :=
  allocate_unique _ _ _ _ _ _ _ accepted formed (by simp [noncell])
