...
theorem enterRegion_preserves_reference_structure (machine : State) (context : Context) (descriptor : RegionId .source)
    ...
    (inputs : ∀ value ∈ arguments, ValueStructure context.source.schemas value.value) :
  ...
  have frameTyped : ValueInventory.All (ValueStructure context.source.schemas)
...
theorem installHandler_preserves_reference_structure (machine : State) (context : Context) (handler : HandlerId .source)
    ...
    (typed : ValueInventory.All (ValueStructure context.source.schemas) machine)
    (inputs : ∀ value ∈ arguments, ValueStructure context.source.schemas value.value)
    (storedTyped : ∀ value ∈ stored, ValueStructure context.source.schemas value.value)
    (outer : ∀ binding ∈ bindings, ValueStructure context.source.schemas binding.located.value) :
    ValueInventory.All (ValueStructure context.source.schemas) after.state := by
  ...
  let property (pair : Heap × List Located) : Prop :=
    ValueInventory.All (ValueStructure context.source.schemas) { machine with heap := pair.1 } ∧
    ∀ value ∈ pair.2, ValueStructure context.source.schemas value.value
  ...
  have selectedTyped : ∀ value ∈ ValueInventory.environment (handlerEnvironment context definition bindings),
      ValueStructure context.source.schemas value := by
    intro value member
  ...
  have framedTyped : ValueInventory.All (ValueStructure context.source.schemas)
...
theorem takeCapture_preserves_reference_structure (machine : State) (context : Context) (token : Located)
    (after : State × Capture) (accepted : takeCapture machine context token = .ok after)
    (typed : ValueInventory.All (ValueStructure context.source.schemas) machine) :
    ValueInventory.All (ValueStructure context.source.schemas) after.1 ∧
    (∀ value ∈ ValueInventory.capture after.2, ValueStructure context.source.schemas value) := by
  ...
  · exact instantiateCapture_preserves_reference_structure _ _ _ _ _ accepted typed storedTyped

theorem resumeValue_preserves_reference_structure (machine : State) (context : Context) (token argument : Located)
    ...
    (typed : ValueInventory.All (ValueStructure context.source.schemas) machine)
    (argumentTyped : ValueStructure context.source.schemas argument.value)
    (successorTyped : match successor with
      | none => True
      | some (_, stored, bindings) => (∀ value ∈ stored, ValueStructure context.source.schemas value.value) ∧
          (∀ binding ∈ bindings, ValueStructure context.source.schemas binding.located.value)) :
    ValueInventory.All (ValueStructure context.source.schemas) after.state := by
  ...
  have capturedTyped := takeCapture_preserves_reference_structure _ _ _ _ captured typed
...
theorem resumeComputation_preserves_reference_structure (machine : State) (context : Context)
...
theorem openRequest_preserves_reference_structure (machine : State) (context : Context)
  ...
  have bodiesTyped : ∀ value ∈ ((operands.drop operation.capability.toList.length).drop 1).take operation.bodies.length,
      ValueStructure context.source.schemas value.value := by
    intro value member
  ...
  · simp only [except_bind_ok] at accepted
    ...
    have activationTyped : ∀ value ∈ ValueInventory.activation selected.activation, ValueStructure context.source.schemas value := by
      intro value member
      apply typed
      simp [ValueInventory.state, reconstructed, ValueInventory.frame, member]
    have outer : ∀ binding ∈ selected.activation.environment, ValueStructure context.source.schemas binding.located.value := by
    ...
    have storedTyped : ∀ value ∈ selected.activation.state, ValueStructure context.source.schemas value.value := by
      intro value member
      apply activationTyped
      simp only [ValueInventory.activation, List.mem_append, List.mem_map]
      exact Or.inr ⟨value, member, rfl⟩
    have outsideTyped : ∀ value ∈ selected.outside.flatMap ValueInventory.frame, ValueStructure context.source.schemas value := by
      intro value member
      apply typed
    ...
    · simp only [except_bind_ok, fromOption_ok] at accepted
      ...
      all_goals
        ...
        have captureTyped : ∀ value ∈ ValueInventory.capture
          ...
          simp only [ValueInventory.capture, List.mem_append, List.mem_map]
          grind only []
        have startingTyped : ValueInventory.All (ValueStructure context.source.schemas)
        ...
        have tokenTyped : ValueStructure context.source.schemas token.value := by
          rw [ValueInventory.allocateObject_result _ _ _ _ _ _ _ allocated]
          apply typed_has_reference_structure
...
/-- Every successful effect-term transition preserves reference structure.
The input assumptions concern only the predecessor state. -/
theorem executeEffectTerm_preserves_reference_structure (machine : State) (context : Context) (after : Transition)
  ...
  case perform operation => exact openRequest_preserves_reference_structure _ _ _ _ _ accepted typed inputs
  case handle handler body arguments stored =>
    split at accepted <;> try contradiction
    rename_i closure rest operandsEqual
    apply installHandler_preserves_reference_structure _ _ _ _ _ _ _ _ accepted typed
    · intro value member
  ...
  case resumeValue token argument =>
    ...
    exact resumeValue_preserves_reference_structure _ _ _ _ _ _ accepted typed
  ...
  case resumeWith token argument handler stored =>
    ...
    apply resumeValue_preserves_reference_structure _ _ _ _ _ _ accepted typed (inputs argumentValue (by simp))
    exact ⟨fun value member => inputs value (by simp [member]), outer⟩
  case resumeComputation token computation =>
    split at accepted <;> try contradiction
    exact resumeComputation_preserves_reference_structure _ _ _ _ _ accepted typed
  case withRegion descriptor body arguments =>
    split at accepted <;> try contradiction
    rename_i closure rest operandsEqual
    exact enterRegion_preserves_reference_structure _ _ _ _ _ _ accepted typed
