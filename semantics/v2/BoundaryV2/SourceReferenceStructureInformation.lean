...
/-- Cleanup preserves catalog paths to reference leaves. Cancellation payloads
introduce no references, and failure entries retain their declared schema. -/
theorem cleanupInformation_preserves_reference_structure (context : Context) (schema : SchemaId .source)
 ...
 : ValueStructure context.source.schemas information := by
  ...
  have unitTyped : ValueStructure context.source.schemas (.scalar unit 0) := .scalar unitAt rfl
  ...
  have cancelledTyped : ∀ value, cancelled = some value → ValueStructure context.source.schemas value ∧ value.schema = reason := by
    intro value found
    obtain ⟨cancellation, cancellationAt, valueAt⟩ := Option.map_eq_some_iff.mp found
    cases cancellation <;> cases valueAt
    · exact ⟨.variant reasonAt (by decide) rfl .blobAny, rfl⟩
    · exact ⟨.variant reasonAt (by decide) rfl .blobAny, rfl⟩
  have optionalTyped : ValueStructure context.source.schemas optionalValue := by
  ...
  have failuresTyped : ValueStructure context.source.schemas (.sequence failures exit.failures) := by
    refine .sequenceAny failuresAt ?_ ?_
    · intro child member
  ...
  have primaryTyped : ∀ value,
      ...
      ValueStructure context.source.schemas value ∧ value.schema = primary := by
    intro value found
  ...
  have finish (primaryValue : SemanticValue)
      (primaryValueTyped : ValueStructure context.source.schemas primaryValue ∧ primaryValue.schema = primary) :
      ValueStructure context.source.schemas (.product schema [primaryValue, optionalValue, .sequence failures exit.failures]) := by
