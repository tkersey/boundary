...
theorem compiled_package
    ...
    (evaluated : Source.ExpressionEvaluation bindings sourceCells.reservations.custody sourceStore expression (.ok inner) sourceEvaluated)
    ...
    let created := createPackage inner owner sourceEvaluated moved handoff sourceCells.reservations.custody
  ...
  let sourceCreated := createPackage inner owner sourceEvaluated moved handoff sourceCells.reservations.custody
  let targetCreated := createPackage (value inner) owner targetEvaluated moved targetHandoff sourceCells.reservations.custody
  have created := package_creation_corresponds (UseScope.PackedControlRelated controlPayloadRelated)
    (fun _ _ body => computation body) inner owner sourceCells.reservations.custody related handoff targetHandoff
...
theorem compiled_unpackage
    ...
    (sourceCells : Cells signature algebra (Source.Computation signature algebra program)) (regions : List (Id .region))
    {sourceStore sourceEvaluated : Source.ControlHeap signature algebra program} {targetStore : Target.ControlHeap signature algebra program}
    (evaluated : Source.ExpressionEvaluation bindings sourceCells.reservations.custody sourceStore expression (.ok (.package token owner inner)) sourceEvaluated)
