...
between a region object's logical identity and its physical heap node. The
cloning case uses the actual copied-node order and fresh region identity map. -/
theorem initialized_execution_preserves_region_identity (context : Context) (arguments : List SemanticValue)
...
theorem initialized_regions_with_equal_identity_are_same_node (context : Context) (arguments : List SemanticValue)
    (before after : State) (events : List Event) (initialized : initial context arguments = .ok before)
    (steps : Steps context before events after) (left right : NodeId) (region : RegionInstanceId)
    (leftDescriptor rightDescriptor : RegionId .source) (leftInvocation rightInvocation : InvocationId)
    (leftParent rightParent : Option RegionInstanceId)
    (leftAt : after.heap.lookup left = some (.region region leftDescriptor leftInvocation leftParent))
    (rightAt : after.heap.lookup right = some (.region region rightDescriptor rightInvocation rightParent)) : left = right :=
  initialized_execution_preserves_region_identity _ _ _ _ _ initialized steps left right region
