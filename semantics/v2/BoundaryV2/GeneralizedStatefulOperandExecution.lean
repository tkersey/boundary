...
theorem compiled_owned_operand_return
    ...
    (evaluated : Source.ExpressionEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore expression (.ok returned) sourceAfter)
...
theorem compiled_owned_operand_failure
    ...
    {sourceStore sourceAfter : Source.ControlHeap signature algebra program}
    {targetStore : Target.ControlHeap signature algebra program}
    (evaluated : Source.ArgumentsEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore body.operandPrefix.arguments (.error fault) sourceAfter)
    (stores : ControlHeapRelated sourceStore targetStore)
    {sourceOutside : Source.Context signature algebra program answer result}
    {targetOutside : Target.Stack signature algebra program answer result}
    (outside : ContextRelated signature algebra program sourceOutside targetOutside) :
  ...
  obtain ⟨count, targetAfter, final, operands, faulted, related⟩ := owned_computation_operand_fault_drains
    (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody body fault evaluated stores
  have reservations : ((cells sourceCells).reservations.withSupport retained) = (sourceCells.reservations.withSupport retained) := Cells.reservations_with_support_mapBodies _ sourceCells retained
  rw [← reservations] at operands
  cases faulted
...
theorem compiled_owned_operand_application
    ...
    {targetStore : Target.ControlHeap signature algebra program}
    (evaluated : Source.ArgumentsEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore (.cons function arguments)
    ...
    ∃ count, 0 < count ∧ Target.ExecutionSteps (retained := retained) (definitions table)
  ...
  | cons functionStep argumentStep =>
    obtain ⟨functionCount, middleTarget, _, functionSteps, middleRelated⟩ := owned_expression_drains
      (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody function
    ...
    obtain ⟨argumentCount, targetEvaluated, argumentSteps, evaluatedRelated⟩ := owned_arguments_drains
      (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody arguments actual
    ...
    · rw [← replaced]
      ...
      have entered := Target.ExecutionSteps.single (retained := retained) (Target.ExecutionStep.control (table := definitions table)
