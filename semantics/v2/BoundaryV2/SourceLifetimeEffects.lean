import BoundaryV2.SourceLifetimeRecords
...
theorem createScope_preserves (machine : State) (context : Context) (invocation : InvocationId)
    ...
    Preserves machine.heap.objects after.heap.objects := by
  ...
  · simp only [except_bind_ok, fromOption_ok, pure, Except.pure, Except.ok.injEq] at accepted
    ...
    have result := move_preserves _ _ _ _ movedOk
...
theorem applyClosure_preserves (machine : State) (context : Context) (closure : Located)
  ...
  obtain ⟨⟨schema, token, reference⟩, _⟩ := lookupObject_reference _ _ _ _ looked
  ...
  | some token =>
    ...
    have first := retire_mutable_preserves _ _ _ _ _ looked rfl retired
...
theorem enterInvocation_preserves (machine : State) (context : Context) (after : Transition)
    (accepted : enterInvocation machine context = .ok after) : Preserves machine.heap.objects after.state.heap.objects := by
  unfold enterInvocation at accepted
  split at accepted <;> try contradiction
  exact invokeFunction_preserves _ _ _ _ _ _ accepted

theorem enterBinding_preserves (machine : State) (context : Context) (after : Transition)
    (accepted : enterBinding machine context = .ok after) : Preserves machine.heap.objects after.state.heap.objects := by
...
theorem instantiateCapture_preserves (machine : State) (context : Context) (saved : Capture) (after : State × Capture)
    (accepted : instantiateCapture machine context saved = .ok after) : Preserves machine.heap.objects after.1.heap.objects := by
  ...
  exact append_preserves _ _

theorem takeCapture_preserves (machine : State) (context : Context) (token : Located) (after : State × Capture)
    (accepted : takeCapture machine context token = .ok after) : Preserves machine.heap.objects after.1.heap.objects := by
  simp only [takeCapture, bind, except_bind_ok] at accepted
  obtain ⟨⟨node, stored⟩, looked, accepted⟩ := accepted
  cases stored <;> try contradiction
  · simp only [except_bind_ok, fromOption_ok, pure, Except.pure, Except.ok.injEq] at accepted
    obtain ⟨store, retired, rfl⟩ := accepted
    exact retire_mutable_preserves _ _ _ _ _ looked rfl retired
  · exact instantiateCapture_preserves _ _ _ _ accepted
...
theorem activateCapture_preserves (machine : State) (context : Context) (saved : Capture)
    ...
    (accepted : activateCapture machine context saved successor = .ok after) : Preserves machine.heap.objects after.heap.objects := by
...
theorem resumeValue_preserves (machine : State) (context : Context) (token argument : Located)
  ...
  have third := temporary_preserves _ _ _ temporaryOk
  have fourth := move_preserves _ _ _ _ moved
  have fifth := finishTemporary_preserves _ _ _ finished
...
theorem installHandler_preserves (machine : State) (context : Context) (handler : HandlerId .source)
  ...
  let property (pair : Heap × List Located) : Prop := Preserves machine.heap.objects pair.1.objects
  ...
  have allPreserved := foldlM_preserves _ _ property (by
    ...
    exact beforePreserved.trans (allocate_preserves _ _ _ _ _ _ _ allocated)) _ _ allocated seed
...
theorem enterRegion_preserves (machine : State) (context : Context) (descriptor : RegionId .source)
  ...
  have first := allocate_preserves _ _ _ _ _ _ _ allocated
...
theorem installProtection_preserves (machine : State) (context : Context) (body cleanup : Located)
  ...
  have first := move_preserves _ _ _ _ moved
...
theorem resumeRelease_preserves (machine : State) (after : AfterRelease) :
    Preserves machine.heap.objects (resumeRelease machine after).state.heap.objects := by
  cases after <;> exact .refl _

theorem finishCleanup_preserves (machine : State) (context : Context) (after : Transition)
    (accepted : finishCleanup machine context = .ok after) : Preserves machine.heap.objects after.state.heap.objects := by
...
theorem discardValues_preserves (machine : State) (context : Context) (after : Transition)
    (accepted : discardValues machine context = .ok after) : Preserves machine.heap.objects after.state.heap.objects := by
  unfold discardValues at accepted
  split at accepted <;> try contradiction
  split at accepted
  · cases accepted; exact resumeRelease_preserves _ _
  · split at accepted
    · cases accepted; exact .refl _
    · simp only [bind, except_bind_ok] at accepted
      ...
      all_goals exact retire_mutable_preserves _ _ _ _ _ looked rfl retired
...
theorem leaveScope_preserves (machine : State) (parent : LexicalScopeId) (invocation : InvocationId)
  ...
  have third := finishTemporary_preserves _ _ _ finishedOk
  exact first.trans (second.trans third)

theorem restoreResumeCaller_preserves (machine : State) (after : Transition)
    (accepted : restoreResumeCaller machine = .ok after) : Preserves machine.heap.objects after.state.heap.objects := by
