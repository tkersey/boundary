...
theorem compiled_cell_read
    ...
    (evaluated : Source.ExpressionEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore reference
    ...
    ∃ count, 0 < count ∧
      Source.CellStep (retained := retained) table ⟨⟨sourceStore, sourceOutside.plug (.evaluate (.cellRead reference) bindings)⟩, sourceCells, regions⟩
  ...
  obtain ⟨count, targetAfter, _, operands, related⟩ := owned_expression_drains
    (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody reference
...
theorem compiled_cell_write
    ...
    (regions : List (Id .region)) (live : region ∈ regions)
    {sourceStore sourceEvaluated : Source.ControlHeap signature algebra program} {targetStore : Target.ControlHeap signature algebra program}
    (evaluated : Source.ArgumentsEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore (.cons reference (.cons replacement .nil))
    ...
    (outside : ContextRelated signature algebra program sourceOutside targetOutside) :
    ∃ count, 0 < count ∧
      Source.CellStep (retained := retained) table ⟨⟨sourceStore, sourceOutside.plug (.evaluate (.cellWrite reference replacement) bindings)⟩, sourceCells, regions⟩
  ...
  obtain ⟨count, targetAfter, operands, related⟩ := owned_two_operands_drains
    (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody reference replacement
...
theorem compiled_cell_allocation
    ...
    (regions : List (Id .region)) (live : region ∈ regions)
    {sourceStore sourceEvaluated : Source.ControlHeap signature algebra program} {targetStore : Target.ControlHeap signature algebra program}
    (evaluated : Source.ArgumentsEvaluation bindings (sourceCells.reservations.withSupport retained).custody sourceStore (.cons regionExpr (.cons valueExpr .nil))
    ...
    (outside : ContextRelated signature algebra program sourceOutside targetOutside) :
    let allocated := Cells.allocate region value sourceCells (referenceNames retained .cell ++ reserved)
    ∃ count, 0 < count ∧
      Source.CellStep (retained := retained) table ⟨⟨sourceStore, sourceOutside.plug (.evaluate (.cellNew regionExpr valueExpr) bindings)⟩, sourceCells, regions⟩
  ...
  dsimp only
  have allocation := Cells.allocate_mapBodies (fun _ _ body => computation body) region value sourceCells (referenceNames retained .cell ++ reserved)
  obtain ⟨count, targetEvaluated, operands, related⟩ := owned_two_operands_drains
    (UseScope.PackedControlRelated controlPayloadRelated) bindings (sourceCells.reservations.withSupport retained).custody regionExpr valueExpr
  ...
  refine ⟨count + 2, by omega, .allocate (retained := retained) evaluated live handoff, ?_, ?_⟩
  · rw [← replaced]
    apply (operands.in_execution (retained := retained) (definitions table) targetOutside (cells sourceCells) regions).trans
    have entered := Target.ExecutionSteps.cons (.cell (Target.CellStep.allocate (retained := retained) (table := definitions table) (reserved := reserved)
