...
/-- The ordinary branch of Source.ExecutionStep uses this actual neutral gate.
...
theorem ordinary_computation_state_step_simulates
  ...
  cases step with
  | operandFault failed =>
    have owned := body.operandPrefix.arguments.owned_evaluation_of_no_closure bindings sourceCells.reservations.custody sourceStore plain
    rw [failed] at owned
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_operand_failure table body bindings _ sourceCells regions owned stores outside
    exact ⟨count, _, positive, steps, related.as_execution⟩
  | returnValue evaluated | primitive evaluated =>
    have owned := Source.Expression.owned_evaluation_of_no_closure _ bindings sourceCells.reservations.custody sourceStore
    ...
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_operand_return table _ bindings _ sourceCells regions owned stores outside
    exact ⟨count, _, positive, steps, related.as_execution⟩
  | bind =>
  ...
  | @call definition reference context arguments bindings values evaluated =>
    have owned := arguments.owned_evaluation_of_no_closure bindings sourceCells.reservations.custody sourceStore plain
    rw [evaluated] at owned
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_named_call table reference arguments bindings values sourceCells regions owned stores outside
    exact ⟨count, _, positive, steps, related.as_execution⟩
  ...
  | matchLeft evaluated selected =>
    ...
    rw [evaluated] at owned
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_match_left table _ _ _ bindings _ _ sourceCells regions owned selected stores outside
    exact ⟨count, _, positive, steps, related.as_execution⟩
  | matchRight evaluated selected =>
    ...
    rw [evaluated] at owned
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_match_right table _ _ _ bindings _ _ sourceCells regions owned selected stores outside
    exact ⟨count, _, positive, steps, related.as_execution⟩
  | @perform effect operation context capability payload bodies bindings attachment payloadValue bodyValues capabilityAt payloadAt bodiesAt =>
    have owned := (Source.Arguments.cons capability (.cons payload bodies)).owned_evaluation_of_no_closure
      bindings sourceCells.reservations.custody sourceStore plain
    simp only [Source.Arguments.evaluate, capabilityAt, payloadAt, bodiesAt] at owned
    obtain ⟨_, count, targetAfter, positive, steps, related⟩ :=
      compiled_owned_operation table operation capability payload bodies bindings attachment payloadValue bodyValues
...
theorem ordinary_program_state_step_simulates
    ...
    (sourceCells : Cells signature algebra (Source.Computation signature algebra program))
    (regions : List (Id .region)) :
    ∃ count targetAfter, Target.ExecutionSteps (definitions table)
  ...
  | evaluate body bindings future =>
    obtain ⟨count, targetAfter, _, steps, matched⟩ :=
      ordinary_computation_state_step_simulates table step neutral outside stores sourceCells regions
  ...
  | bind body bindings inner induction =>
    cases step with
    | bindValue =>
      obtain ⟨count, steps⟩ := inner.returned_state_drains (definitions table) _ rfl targetStore (cells sourceCells) regions
    ...
    | bindFault =>
      obtain ⟨count, steps⟩ := inner.failed_state_drains (definitions table) _ rfl targetStore (cells sourceCells) regions
...
theorem cell_state_step_simulates (table : Source.Definitions signature algebra program)
    ...
    {target : Target.State signature algebra program result}
    (step : Source.CellStep table source after) (related : ExecutionStateRelated source target) :
...
theorem control_state_step_simulates (table : Source.Definitions signature algebra program)
    ...
    (sourceCells : Cells signature algebra (Source.Computation signature algebra program))
    (regions : List (Id .region))
    (step : Source.OwnedStep table sourceCells.reservations source after)
  ...
  | @ordinary type first last sourceStore ordinary neutral =>
    rw [related.target_state]
    exact ordinary_program_state_step_simulates table related.computation ordinary neutral .done related.store sourceCells regions
  | @resume context mode effect input body result sourceStore value evaluated view after use continuation response bindings sourceOutside operands accepted =>
    ...
    obtain ⟨targetAfter, count, _, matched, _, steps⟩ :=
      compiled_owned_resumption table sourceCells.reservations use continuation response bindings view value operands related.store outside accepted
    ...
    exact ⟨count, _, steps.in_execution (cells sourceCells) regions,
  ...
  | @successor context effect input body answer result sourceStore value evaluated view after use continuation response returned clauses bindings sourceOutside operands accepted =>
    ...
    rw [configuration]
    obtain ⟨targetAfter, count, _, matched, _, steps⟩ :=
      compiled_owned_successor table sourceCells.reservations use continuation response returned clauses bindings view value
...
theorem stateful_execution_step_simulates (table : Source.Definitions signature algebra program)
    {source after : Source.State signature algebra program result}
    {target : Target.State signature algebra program result}
    (step : Source.ExecutionStep table source after) (related : ExecutionStateRelated source target) :
    ∃ count targetAfter, Target.ExecutionSteps (definitions table) target count targetAfter ∧
      ExecutionStateRelated after targetAfter := by
  cases step with
  ...
  | @performOperands effect context result sourceStore attachment afterStore regions operation payloadValue bodyValues capability payload bodies bindings sourceOutside sourceCells operands =>
    ...
    rw [configuration]
    obtain ⟨_, count, targetAfter, _, steps, matched⟩ :=
      compiled_owned_operation table operation capability payload bodies bindings attachment payloadValue bodyValues sourceCells regions
...
theorem finite_stateful_execution_preserved (table : Source.Definitions signature algebra program)
    {source after : Source.State signature algebra program result}
    {target : Target.State signature algebra program result}
    (steps : Source.ExecutionSteps table source sourceCount after)
    (related : ExecutionStateRelated source target) :
    ∃ count targetAfter, Target.ExecutionSteps (definitions table) target count targetAfter ∧
      ExecutionStateRelated after targetAfter := by
...
theorem stateful_observation_preserved (table : Source.Definitions signature algebra program)
    ...
    {target : Target.State signature algebra program result}
    (related : ExecutionStateRelated source target)
    (observed : Source.StateObserves table source final observation) :
  ...
  obtain ⟨targetCount, targetAfter, targetSteps, matched⟩ := finite_stateful_execution_preserved table sourceSteps related
  obtain ⟨targetFinal, targetObservation, targetObserved, same⟩ := stateful_head_observation_preserved table matched sourceHead
