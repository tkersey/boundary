import BoundaryV2.SourceValueLinearity
...
theorem closure_creation_preserves_linearity (state : State) (context : Context)
    ...
    (typed : ValueInventory.All (Linear) state)
    (inputs : ∀ value ∈ values, Linear value.value) :
...
theorem makeClosure_preserves_linearity (state : State) (context : Context)
    (schema : SchemaId .source) (function : FunctionId .source)
    (bindings : Environment) (after : Transition)
    (accepted : makeClosure state context schema function bindings = .ok after)
    (typed : ValueInventory.All (Linear) state)
    (inputs : ∀ binding ∈ bindings, Linear binding.located.value) :
    ValueInventory.All (Linear) after.state := by
  simp only [makeClosure, bind, except_bind_ok] at accepted
  obtain ⟨values, captured, accepted⟩ := accepted
  exact closure_creation_preserves_linearity state context schema function values after accepted typed
...
/-- Expression entry preserves per-value uniqueness. Literal values contain
no owning tokens, and newly allocated closures return one fresh reference. -/
theorem enterExpression_preserves_linearity (state : State) (context : Context) (after : Transition)
  ...
  have bindingsTyped : ∀ binding ∈ bindings, Linear binding.located.value := by
  ...
  | literal constant =>
    ...
    exact ValueInventory.scopedValue_preserves_all _ _ _ accepted _ typed
      (by simp [Linear, (external_value_has_no_runtime_handles _ _
        ((checked_context_constants_are_external context contextTyped) value (List.mem_of_getElem? foundValue))).2])
  | lambda function =>
    exact makeClosure_preserves_linearity state context schema function bindings after accepted typed bindingsTyped
...
theorem heapPrimitive_preserves_linearity (machine : State) (context : Context)
  ...
  case computation =>
    ...
    subst expected
    exact closure_creation_preserves_linearity _ _ _ _ _ _ accepted typed inputs
  case cellNew =>
    split at accepted <;> try contradiction
  ...
  case cellSet =>
    ...
    exact ValueInventory.scopedValue_preserves_all _ _ _ accepted _ storeTyped (by simp [Linear, ownedTokens])
  case package =>
    split at accepted <;> try contradiction
    rename_i value
    simp only [except_bind_ok] at accepted
...
theorem executePrimitive_preserves_linearity (machine : State) (context : Context) (after : Transition)
  ...
  have constantsTyped : ∀ value ∈ context.executionConstants, Linear value :=
    fun value member => by simp [Linear, (checked_execution_constants_have_no_handles context contextTyped value member).2]
  ...
  · rename_i value evaluated
    have valueTyped := PrimitiveLinearity.committed_primitive_result_unique _ _ _ _ _ _ _ _ contextTyped evaluated accepted
    exact ValueInventory.commitPure_preserves_all _ _ _ _ _ accepted _ typed valueTyped
  · exact heapPrimitive_preserves_linearity _ _ _ _ _ _ _ accepted typed inputs
