...
runs="${SHIFT_MATRIX_STABILITY_RUNS:-5}"

case "$runs" in
  ''|*[!0-9]*)
    echo "SHIFT_MATRIX_STABILITY_RUNS must be a positive integer" >&2
    exit 1
    ;;
esac

[ "$runs" -gt 0 ] || {
  echo "SHIFT_MATRIX_STABILITY_RUNS must be greater than zero" >&2
  exit 1
}
...
[ "$repo_state" = "clean" ] || {
  echo "effect-matrix stability must run on a clean tree" >&2
  exit 1
}

tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/shift-effect-matrix-stability.XXXXXX")"
...

{
  ...
  for (i = 1; i <= NF; i += 1) {
    ...
    else if (kv[1] == "raw_median_ns") raw = kv[2] + 0;
    else if (kv[1] == "effect_median_ns") effect = kv[2] + 0;
  }
  if (lane == "" || raw == 0) next;
