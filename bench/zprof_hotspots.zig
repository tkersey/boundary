...
fn runWriterEffect(runtime: *boundary.Runtime, instance: *WriterInstance, profiler: *Zprof(.{})) !usize {
    const allocator = profiler.allocator();

    // Exclude one-time runtime/handler bootstrap from the steady-state profile.
    const warmup = preserveValue(try boundary.effect.writer.handle(usize, usize, runtime, instance, allocator, struct {
        /// Append one fixed number of writer items.
        pub fn body(comptime Cap: type, ctx: anytype) boundary.ResetError(NoError)!usize {
            var current: usize = 0;
            while (current < writer_items_per_body) : (current += 1) {
                try boundary.effect.writer.tell(Cap, ctx, current + 1);
            }
            return 0;
        }
    }));
...
fn runResourceEffect(runtime: *boundary.Runtime, instance: *ResourceInstance, profiler: *Zprof(.{})) !usize {
    resource_synth.current_base = profile_iterations * resource_items_per_body;
    resource_synth.acquire_count = 0;
    resource_synth.release_sink = 0;
    _ = preserveValue(try boundary.effect.resource.handle(usize, runtime, instance, struct {
        ...
        pub fn body(comptime Cap: type, ctx: anytype) boundary.ResetError(NoError)!usize {
            ...
            var current: usize = 0;
            while (current < resource_items_per_body) : (current += 1) {
                lane_checksum +%= try boundary.effect.resource.acquire(Cap, ctx);
            }
    ...
    while (iteration < profile_iterations) : (iteration += 1) {
        ...
        checksum +%= preserveValue(try boundary.effect.resource.handle(usize, runtime, instance, manager, body));
    }
    return checksum;
}
...
pub fn main(init: std.process.Init) anyerror!void {
    ...
    var effect_writer_runtime = boundary.Runtime.init(effect_writer_profiler.allocator());
    ...
    var effect_resource_runtime = boundary.Runtime.init(effect_resource_profiler.allocator());
