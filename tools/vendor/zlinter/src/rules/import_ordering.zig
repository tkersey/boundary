...
fn deinitScopedImports(gpa: std.mem.Allocator, scoped_imports: *std.AutoHashMap(Ast.Node.Index, ImportsQueueLinesAscending)) void {
    var values = scoped_imports.valueIterator();
...
fn resolveScopedImports(
    doc: *const zlinter.session.LintDocument,
    gpa: std.mem.Allocator,
) !std.AutoHashMap(Ast.Node.Index, ImportsQueueLinesAscending) {
    const tree = doc.handle.tree;
    ...
    var scoped_imports: std.AutoHashMap(Ast.Node.Index, ImportsQueueLinesAscending) = .init(gpa);
