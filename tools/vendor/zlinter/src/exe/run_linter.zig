...
fn run(
    ...
    var file_lint_problems = std.AutoHashMap(
        u32,
        []zlinter.results.LintResult,
    ).init(gpa);
    defer {
        var values = file_lint_problems.valueIterator();
        while (values.next()) |results| {
...
fn runLinterRules(
    ...
    printer: *zlinter.rendering.Printer,
    timer: *Timer,
    file_lint_problems: *std.AutoHashMap(u32, []zlinter.results.LintResult),
    args: zlinter.Args,
) !void {
...
fn runFormatter(
    ...
    var values_flatten = file_lint_problems.valueIterator();
    while (values_flatten.next()) |results| {
