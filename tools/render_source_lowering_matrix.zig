...
fn outputPath() []const u8 {
    return "docs/source_lowering_matrix.json";
}

fn usage() noreturn {
    std.debug.print("usage: shift-source-lowering-matrix <write|check>\n", .{});
    std.process.exit(1);
}

fn render(list: *std.ArrayList(u8), allocator: std.mem.Allocator) !void {
    try list.appendSlice(allocator, "{\n");
    try list.appendSlice(allocator, "  \"wave\": \"source_lowering_canonical\",\n");
    try list.appendSlice(allocator, "  \"lowering_surface\": \"canonical_source_validated\",\n");
    try list.appendSlice(allocator, "  \"cases\": [\n");
    for (source_lowering_registry.cases, 0..) |case, idx| {
...
/// Render or check the source-lowering matrix artifact.
pub fn main() anyerror!void {
    ...
    if (!std.mem.eql(u8, actual, rendered.items)) {
        std.debug.print("source-lowering matrix drift: {s}\n", .{outputPath()});
        return error.SourceLoweringMatrixDrift;
    }
}
