...
/// ProgramPlan authoring/resource failures surface as Zig errors; binary decode
/// verdicts use the narrower DecodeResultError domain.
...
test "ArtifactV1 validation preserves duplicate output label reason" {
    const allocator = std.testing.allocator;
    const manifest_fingerprint = buildFingerprintFromSeed("duplicate-output-plan");
    const build_fingerprint = try buildFingerprintForCapabilitiesForArtifactVersion(
        allocator,
        artifact_version_current,
        manifest_fingerprint,
    ...
    const plan_functions = [_]program_plan.FunctionPlan{.{
        .symbol_name = "entry",
        .parameter_count = 0,
        .first_requirement = 0,
        .requirement_count = 0,
    ...
    try std.testing.expectError(error.DuplicateOutputLabel, validateWithProgramPlan(artifact, allocator, invalid_plan, null));
}

test "ArtifactV1 validation preserves too many function outputs reason" {
    const allocator = std.testing.allocator;
    const manifest_fingerprint = buildFingerprintFromSeed("too-many-output-plan");
    const build_fingerprint = try buildFingerprintForCapabilitiesForArtifactVersion(
        allocator,
        artifact_version_current,
        manifest_fingerprint,
    ...
    const output_count = program_plan.max_validated_function_outputs + 1;
    const outputs = try allocator.alloc(program_plan.OutputPlan, output_count);
    ...
    try std.testing.expectError(error.TooManyFunctionOutputs, validateWithProgramPlan(artifact, allocator, invalid_plan, null));
}
