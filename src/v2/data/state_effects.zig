...
pub fn CheckFor(comptime Program: type, comptime State: type) type {
    ...
    return struct {
        ...
        program: Program,
        state: State,
        activations: []const ?@import("process_state.zig").Activation = &.{},
        captures: []const ?p.Id,
        ...
        fn node(self: Self, ref: g.NodeRef) Error!g.Node {
        ...
        fn block(self: Self, id: p.Id) Error!Block {
            if (id >= self.program.blocks.len) return error.InvalidReference;
            return self.program.blocks[@intCast(id)];
        }

        fn scopeEffects(self: Self, source: p.Id, cleanup: bool, effect: p.Id) Error!bool {
            ...
            const signature = try contracts.computation(self.program, try contracts.programSlotSchema(self.program, code, operand));
            return contracts.containsEffect(signature.effects, effect);
        }

        pub fn requireEffects(self: *Self, parent: ?g.NodeRef, effects: []const p.Id) Error!void {
            for (effects) |effect| if (!try self.allows(parent, effect)) return error.InvalidEffect;
        }

        fn allows(self: *Self, parent: ?g.NodeRef, effect: p.Id) Error!bool {
            var cursor = parent;
        ...
        pub fn attachment(self: *Self, ref: g.NodeRef) Error!void {
        ...
        const Origins = struct {
            ...
            fn init(check: *Self, effect: p.Id) Error!Origins {
                var self: Origins = .{
                    .effect = effect,
                ...
                for (check.activations) |activation| if (activation) |frame|
                    for (frame.bindings) |binding| try self.collect(g.Value, check, binding.value);
            ...
            fn collect(self: *Origins, comptime T: type, check: *Self, value: T) Error!void {
                if (T == g.Value) {
            ...
            fn merge(self: Origins, check: *Self, into: []usize, ref: g.NodeRef, mask: ?g.NodeRef) Error!bool {
            ...
            fn valueOrigins(self: Origins, check: *Self, into: []usize, item: g.Value) Error!bool {
            ...
            fn values(self: Origins, comptime T: type, check: *Self, into: []usize, input: T) Error!bool {
            ...
            fn child(self: Origins, check: *Self, id: usize, into: []usize, mask: ?g.NodeRef) Error!bool {
            ...
            fn delimiter(self: Origins, check: *Self, id: usize, into: []usize) Error!bool {
                const attached = check.state.nodes[id].attachment;
                const definition = try check.handler(attached.handler);
                const reinstated = attached.phase == .active or blk: {
                ...
                if (reinstated) changed = try self.merge(check, into, attached.handler, null) or changed;
                return changed;
            }

            fn solve(self: *Origins, check: *Self) Error!void {
                ...
                while (changed) {
                    ...
                    for (check.state.nodes, 0..) |record, id| {
                        ...
                        if (check.activations.len != 0) if (check.activations[id]) |frame| {
                            for (frame.bindings) |binding| changed = try self.valueOrigins(check, into, binding.value) or changed;
                        };
