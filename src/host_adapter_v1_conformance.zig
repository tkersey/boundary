...
pub fn assertSequentialRequestIds(entries: []const host.HostLogEntryV1) anyerror!void {
...
/// Assert that one host log entry preserves the requested tool id and op name.
pub fn assertToolCallShape(entry: host.HostLogEntryV1, tool_id: []const u8, op_name: []const u8) anyerror!void {
    ...
    const result = entry.result.body.success;
