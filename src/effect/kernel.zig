...
pub fn Family(comptime State: type, comptime Answer: type, comptime ErrorSet: type) type {
    const PromptType = raw.Prompt(.resume_then_transform, Answer, Answer, ErrorSet);
    return struct {
        ...
        pub const context = struct {
            ...
            pub inline fn perform(self: *@This(), comptime Op: type, payload: Op.Payload) raw.ResetError(ErrorSet)!Op.Resume {
                _ = self;
                const handler = struct {
                    ...
                    inline fn callResume(frame: *Frame, payload_value: Op.Payload) raw.ResetError(ErrorSet)!Op.Resume {
                        ...
                        switch (@typeInfo(ReturnType)) {
                            .error_union => |error_union| {
                                if (error_union.error_set != raw.ResetError(ErrorSet) or error_union.payload != Op.Resume) {
                ...
                return try raw.shift(Op.Resume, PromptType, &active_frame.?.prompt, handler);
            }
        };
