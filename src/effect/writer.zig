...
/// Handler writer handle used by `boundary.effect handlers`.
...
/// Descriptor value used by `boundary.effect handlers` for the built-in writer family.
...
/// Create one handler writer descriptor for `boundary.effect handlers`.
