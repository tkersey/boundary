...
pub fn compile(allocator: std.mem.Allocator, kind: Kind, options: data.coalescing.Options) !source.Compiled {
    ...
    for (0..2) |id| {
        ...
        const assignments = try a.alloc(ir.Assignment, if (kind == .swap) 2 else 3);
        const order: []const usize = if (kind == .swap) &.{ 1, 0 } else &.{ 1, 2, 0 };
        ...
        operations[0] = .{ .destination = 3, .opcode = .product, .operands = inputs };
