...
try {
  ...
  await writeFile(join(directory,'build.zig.zon'),`.{ .name=.consumer, .version="0.0.0", .fingerprint=0x8137d8fc0ed9b57d, .dependencies=.{ .boundary=.{.url="file://${archive}",.hash="${hash}"}}, .paths=.{""} }`);
