...
if (!bundle || !imagePath || !sha256 || !nativePath)
  throw new Error('usage: node authoring_shallow_world.mjs BUNDLE IMAGE SHA NATIVE');
...
for (const inputValue of [false, true]) {
  ...
  const input = encodeInput({ image, initialArgs: Uint8Array.of(inputValue ? 1 : 0) });
  ...
  assert.equal(outcome.kind, inputValue ? 'failed' : 'completed');
  if (!inputValue) assert.deepEqual(outcome.value, Uint8Array.of(1));
}
console.log(JSON.stringify({ falseInput: 'completed true',
  trueInput: 'failed', nativeAgreement: true }));
