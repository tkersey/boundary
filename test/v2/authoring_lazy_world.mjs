...
for (const demandFailure of [false, true]) {
  ...
  assert.equal(outcome.kind, demandFailure ? 'failed' : 'completed');
  if (!demandFailure) assert.deepEqual(outcome.value,
    Uint8Array.of(42, 0, 0, 0, 0, 0, 0, 0));
}
console.log(JSON.stringify({ unusedFailure: 'completed 42',
  demandedFailure: 'failed', nativeAgreement: true }));
