...
const example_resource_basic = @import("promoted_example_resource_basic");
...
const example_writer_basic = @import("promoted_example_writer_basic");

const CanonicalSourceCase = struct {
    case_id: []const u8,
...
fn expectCanonicalSourceCase(comptime canonical_case: CanonicalSourceCase) !void {
    var direct_buffer: [2048]u8 = undefined;
    var direct_writer = std.Io.Writer.fixed(&direct_buffer);
    try canonical_case.Runner.run(&direct_writer);
...
test "public example and built-in effect source-lowering cohort stays source-backed and canonical" {
    try expectCanonicalSourceCase(.{
