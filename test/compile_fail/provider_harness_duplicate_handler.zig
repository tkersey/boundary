...
const Body = struct {
    pub const compiled_plan = plan("provider-harness-duplicate-handler");
};
const Program = boundary.program("provider-harness-duplicate-handler", struct {}, Body);
const Site = Program.protocol.operationSite("protocol", "step", 0);

fn handle() void {
    // Handler body is irrelevant; this fixture fails during harness declaration.
}

const Harness = Program.Exchange.ProviderHarness(.{
    .label = "duplicate-provider",
    .entries = .{
        Program.Exchange.ProviderHandler.operation(Site, handle, .{}),
        Program.Exchange.ProviderHandler.operation(Site, handle, .{}),
    },
});
