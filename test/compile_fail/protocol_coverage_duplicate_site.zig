...
const Body = struct {
    pub const compiled_plan = plan("protocol-coverage-duplicate-site");
};
const Program = boundary.program("protocol-coverage-duplicate-site", struct {}, Body);
const Site = Program.protocol.operationSite("protocol", "step", 0);

comptime {
    Program.protocol.assertOperationSitesCovered(.{ Site, Site });
}
