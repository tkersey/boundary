...
const Body = struct {
    pub const compiled_plan = plan("interpreter-coverage-fake-interpreter");
};
const Program = boundary.program("interpreter-coverage-fake-interpreter", struct {}, Body);
...
comptime {
    Program.protocol.assertAllSitesCoveredBy(FakeInterpreter);
}
