...
fn plan(comptime label: []const u8, comptime hash: u64) ability.ir.ProgramPlan {
...
const Body = struct {
    pub const compiled_plan = plan("interpreter-foreign-site", 93);
};
const ForeignBody = struct {
    pub const compiled_plan = plan("interpreter-foreign-site", 94);
};
const Program = ability.program("interpreter-foreign-site", struct {}, Body);
const ForeignProgram = ability.program("interpreter-foreign-site", struct {}, ForeignBody);
const ForeignSite = ForeignProgram.protocol.operationSite("protocol", "step", 0);

fn handle(_: anytype, _: anytype, _: Program.Handler.Control) !Program.Handler.Outcome(ForeignSite) {
    return Program.Handler.@"resume"(ForeignSite, @as(i32, 1));
}

const Handler = Program.Handler.operation(ForeignSite, handle);
