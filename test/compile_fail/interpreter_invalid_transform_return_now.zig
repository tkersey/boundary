...
const Body = struct {
    pub const compiled_plan = plan("interpreter-invalid-transform-return-now");
};
const Program = boundary.program("interpreter-invalid-transform-return-now", struct {}, Body);
const Site = Program.protocol.operationSite("protocol", "step", 0);

comptime {
    _ = Program.Handler.returnNow(Site, @as(i32, 1));
}
