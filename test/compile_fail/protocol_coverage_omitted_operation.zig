...
const Body = struct {
    pub const compiled_plan = plan("protocol-coverage-omitted-operation");
};
const Program = boundary.program("protocol-coverage-omitted-operation", struct {}, Body);

comptime {
    Program.protocol.assertOperationSitesCovered(.{});
}
