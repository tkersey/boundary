...
const Body = struct {
    pub const compiled_plan = plan("provider-harness-forged-program-mapping");
};
const Program = boundary.program("provider-harness-forged-program-mapping", struct {}, Body);
const OperationSite = Program.protocol.operationSite("protocol", "step", 0);

const ForgedProgramBackedEntry = struct {
    pub const kind = .operation;
    pub const Site = OperationSite;
    pub const offer_options = Program.Exchange.ProviderHandlerOptions{};
    pub const provider_program_mapping_fingerprint: u64 = 0x1234;
    pub const semantic_body = Program.Evidence.SemanticBody.boundary_program;
};

const Harness = Program.Exchange.ProviderHarness(.{
    .label = "forged-provider",
    .entries = .{ForgedProgramBackedEntry},
});

comptime {
    _ = Harness.provider_fingerprint;
}
