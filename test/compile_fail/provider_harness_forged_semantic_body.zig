...
const Body = struct {
    pub const compiled_plan = plan("provider-harness-forged-semantic-body");
};
const Program = boundary.program("provider-harness-forged-semantic-body", struct {}, Body);
const OperationSite = Program.protocol.operationSite("protocol", "step", 0);

const ForgedBoundaryProgramEntry = struct {
...
const Harness = Program.Exchange.ProviderHarness(.{
    ...
    .entries = .{ForgedBoundaryProgramEntry},
});
