...
const Program = ability.program("provider-harness-forged-semantic-body", struct {}, Body);
const OperationSite = Program.protocol.operationSite("protocol", "step", 0);

const ForgedAbilityProgramEntry = struct {
...
const Harness = Program.Exchange.ProviderHarness(.{
    ...
    .entries = .{ForgedAbilityProgramEntry},
});
