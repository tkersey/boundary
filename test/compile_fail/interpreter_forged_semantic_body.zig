...
const Body = struct {
    pub const compiled_plan = plan("interpreter-forged-semantic-body");
};
const Program = boundary.program("interpreter-forged-semantic-body", struct {}, Body);
const OpSite = Program.protocol.operationSite("protocol", "step", 0);

fn handle(_: anytype, _: anytype, _: Program.Handler.Control) !Program.Handler.Outcome(OpSite) {
    return Program.Handler.@"resume"(OpSite, @as(i32, 1));
}

const ForgedHandler = struct {
    pub const kind = .operation;
    pub const Site = OpSite;
    pub const function = handle;
    pub const semantic_body = Program.Evidence.SemanticBody.boundary_program;
};

const Interpreter = Program.Interpreter(.{ForgedHandler});
