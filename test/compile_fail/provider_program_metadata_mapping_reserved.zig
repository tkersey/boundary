...
fn sourcePlan(comptime label: []const u8) boundary.ir.ProgramPlan {
...
const SourceBody = struct {
    pub const compiled_plan = sourcePlan("provider-program-metadata-mapping-reserved");
};
const SourceProgram = boundary.program("provider-program-metadata-mapping-reserved", struct {}, SourceBody);
const Site = SourceProgram.protocol.operationSite("protocol", "step", 0);

const HandlerBody = struct {
    pub const compiled_plan = handlerPlan("provider-program-metadata-mapping-reserved-handler");
};
const HandlerProgram = boundary.program("provider-program-metadata-mapping-reserved-handler", struct {}, HandlerBody);

const Decl = SourceProgram.Exchange.ProviderHandler.program(.{
    .label = "metadata-mapping-reserved",
    .op = Site,
    .program = HandlerProgram,
    .map_request = .payload_and_metadata_to_args,
