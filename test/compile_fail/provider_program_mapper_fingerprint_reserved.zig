...
const SourceBody = struct {
    pub const compiled_plan = sourcePlan("provider-program-mapper-fingerprint-reserved");
};
const SourceProgram = boundary.program("provider-program-mapper-fingerprint-reserved", struct {}, SourceBody);
const Site = SourceProgram.protocol.operationSite("protocol", "step", 0);

const HandlerBody = struct {
    pub const compiled_plan = handlerPlan("provider-program-mapper-fingerprint-reserved-handler");
};
const HandlerProgram = boundary.program("provider-program-mapper-fingerprint-reserved-handler", struct {}, HandlerBody);

const Decl = SourceProgram.Exchange.ProviderHandler.program(.{
    .label = "mapper-fingerprint-reserved",
    .op = Site,
    .program = HandlerProgram,
    .map_request = .unit_args,
    .map_result = .result_to_resume,
    .mapper_fingerprint = 0x5158,
});
