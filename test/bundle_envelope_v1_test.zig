...
test "BundleEnvelopeV1 default compile path uses the derived capability fingerprint" {
    ...
    const expected = try shift_vm.artifact.buildFingerprintForCapabilities(
        std.testing.allocator,
    ...
    try std.testing.expectEqualSlices(u8, bytes, imported.artifact_bytes);
    try std.testing.expect(!std.mem.eql(u8, &expected, &shift_vm.artifact.defaultBuildFingerprint()));

    const wrong = shift_vm.artifact.defaultBuildFingerprint();
    try std.testing.expectError(error.BuildFingerprintMismatch, shift_vm.bundle.importBundle(std.testing.allocator, envelope_bytes, wrong));
}
