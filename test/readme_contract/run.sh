...
grep -q 'effects-library surface' "$readme"
grep -q 'lexical execution through' "$readme"
...
grep -q 'compatibility-only' "$readme"
...
grep -q '`shift_vm.durable`, and `shift_vm.ErrorWitnessV1` remain supported' "$readme"
