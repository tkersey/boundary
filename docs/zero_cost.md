...
- Every `resumeWith` performs another context switch back into the captured frame.
...
- Early-exit handlers can avoid the resume path, but they still pay the initial capture and handler dispatch cost.
