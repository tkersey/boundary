...
- derived provider offers with `semanticBody`, `hostIntrinsicRef`, and
  `programBodyRef`

TreatyResolver can apply defunctionalization policy while selecting routes. With
preference enabled, program-backed providers rank ahead of function-backed host
intrinsics, and static residualized or pipeline morphisms rank ahead of dynamic
host mapper functions. With strict policy, intrinsic providers and intrinsic
morphism mappers are blocked.
