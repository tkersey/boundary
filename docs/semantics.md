...
3. `shift(Spec, request)` allocates a one-shot token record, suspends the current fiber, and returns control to the caller as `Outcome.pending`.
4. The caller owns the returned pending owner and must resolve it or explicitly promote it to `EscapedToken`.
...
- `error.TokenAliased`: a copied escaped-owner alias attempted to resolve a one-shot owner held by a different handle.
