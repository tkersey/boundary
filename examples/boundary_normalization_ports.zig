...
const compiled = semantic.finish(.{
    .label = "boundary-normalization-ports-residual",
    .ir_hash = 0x6e706f7274730001,
    .entry = "run",
...
}) catch |err| @compileError("invalid normalization ports residual: " ++ @errorName(err));

const Program = boundary.program("boundary-normalization-ports-residual", Handlers, struct {
...
const intrinsic_ref = Program.Evidence.refFor(Program.Evidence.domains.host_intrinsic, 0x6e70_9001, .{ .label = "normalization-human-approval" });
...
const port = Closure.WorldPort.init(.{
    .label = "normalization-human-approval-port",
    .kind = .host_human,
...
const closure_graph = Closure.Graph.init("boundary-normalization-ports-graph", &.{}, &.{}, &.{});
...
const Target = blk: {
    ...
    break :blk Elaboration.Target.compileComptime(.{
        .label = "boundary-normalization-ports-target",
