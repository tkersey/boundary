...
pub fn runBody(eff: anytype) ![]const u8 {
    ...
    const label = try helpers.classify("cross-selected", count, eff);
...
/// Return the source path captured by this cross-file helper value-flow example module.
pub fn loweringSourcePath() [:0]const u8 {
    return "examples/open_row_helper_value_flow_cross.zig";
}
...
/// Write the cross-file helper value-flow transcript.
