...
/// Result of driving a delimiter until completion or pending ownership.
...
fn InternalOutcome(comptime Spec: type) type {
    return union(enum) {
        cancelled: void,
        complete: AnswerOf(Spec),
        pending: Pending(Spec),
...
pub fn EscapedOwner(comptime Spec: type) type {
    ...
    return if (comptime resumeIsVoid(Spec)) struct {
        const Self = @This();
        ...
        fn abandon(self: *Self) ResetError(ErrorSetOf(Spec))!void {
            const record = try self.prepare();
            ...
            const outcome = try driveDelimiterInternal(Spec, record.target_frame);
            switch (outcome) {
                .cancelled => return,
...
fn driveDelimiterInternal(comptime Spec: type, frame: *ResetFrame(Spec)) ResetError(ErrorSetOf(Spec))!InternalOutcome(Spec) {
...
fn driveDelimiter(comptime Spec: type, frame: *ResetFrame(Spec)) ResetError(ErrorSetOf(Spec))!Outcome(Spec) {
    return switch (try driveDelimiterInternal(Spec, frame)) {
        .complete => |answer| .{ .complete = answer },
        .pending => |pending| .{ .pending = pending },
        .cancelled => error.CancellationRecovered,
    };
}
...
pub fn reset(
    ...
    try runtime.ensureThread();
    const frame = try ResetFrame(Spec).create(runtime, prompt, body);
...
pub fn shift(
    ...
    const wanted_prompt = try promptToken(prompt);
    ...
    const frame: *ResetFrame(Spec) = @fieldParentPtr("base", target_fiber);
